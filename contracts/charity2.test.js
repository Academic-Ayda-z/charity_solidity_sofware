...
//      let amount=web3.utils.toWei('1','ether');
      ...
      await Charity.make_donation(0,{from:accounts[0],value: web3.toWei(3, "ether")});

      const donator1 = await Charity.creatDonator('donator 1', web3.utils.toWei('4', 'ether'),0,{ from:accounts[3]});
      await Charity.make_donation(1,{from:accounts[1],value: web3.toWei(4, "ether")});
 ...
 it("donarotVotesRequset",async () => {
  ...
  it("beneficiary buy product",async () => {
    await Charity.benificiaryBuyProduct(0,0,0,1,{from:accounts[1],value: web3.toWei(1, "ether")});
  });

  it("withdraw from the store",async () => {
    await Charity.withdraw(0,{from:accounts[0],value: web3.toWei(1, "ether")});
  });
});
